...
def get_challenger_action(my_snake, opponent_snake, foods, grid_width, grid_height):
    ...
    if opponent_alive:
        max_opponent_length = max(max_opponent_length, opponent_snake.length)

    if opponent_alive and my_snake.length > max_opponent_length:
    ...
    if foods and len(possible_actions) > 1:
        return random.choice([a for a in possible_actions if my_snake.DIRECTIONS_MAP[a] != my_snake.DIRECTIONS_MAP[my_snake.direction_idx]])
