...
def get_challenger_action(my_snake, opponent_snake, foods, grid_width, grid_height):
    ...
    opponent_head = opponent_snake.get_head_position() if opponent_snake else None
    ...
    # If opponent is moving towards us, try to move away
    opponent_dx, opponent_dy = my_snake.DIRECTIONS_MAP[opponent_snake.direction_idx]
    new_opponent_distance = abs((head_pos[0] + opponent_dx) % grid_width - opponent_head[0]) + abs((head_pos[1] + opponent_dy) % grid_height - opponent_head[1])
    if new_opponent_distance < distance_to_opponent:
        return opponent_distances[0][1]

    # If opponent is moving away, try to follow
    # Calculate the angle between the opponent's direction and the line connecting our heads
    angle = math.atan2(opponent_dy, opponent_dx) - math.atan2(dy_op, dx_op)
    if abs(angle) > math.pi / 4:
        # If the angle is greater than 45 degrees, try to move towards the opponent
        return opponent_distances[0][1]
    else:
        # If the angle is less than or equal to 45 degrees, try to move away from the opponent
        return opponent_distances[1][1]